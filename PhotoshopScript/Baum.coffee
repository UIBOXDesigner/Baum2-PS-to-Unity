# ==========================================
# Baum PSD to Unity UI Converter v0.7.0
...
# 
# 使用示例：
# ui_btn_ok_normal@center → Unity Button (pivot=center, state=normal)
# ui_img_coin@stretchx → Unity Image (stretchx=true)
# ui_panel_main@bottom → Unity Panel (pivot=bottom)
# ==========================================
...
class Baum
  ...
  runOneFile: (after_close) =>
    ...
    @removeCommentoutLayers(copiedDoc, copiedDoc) # blend处理完后再删除
...
class PsdToJson
    ...
    if layer.kind == LayerKind.TEXT
      ...
      if text.kind != TextType.PARAGRAPHTEXT
        try
          ...
          alert("文本图层转换失败，请检查是否使用了【仿粗体/仿斜体】！\n出问题的图层名: " + layer.name)
          throw e
        textType = 'point'
  ...
  groupToHash: (document, name, opt, layer) ->
    ...
    else if name.startsWith('ui_slider_') || name.startsWith('ui_progress_')
      hash = { type: 'Slider' }
      hash['scroll'] = opt['scroll'] if opt['scroll']
    else if name.startsWith('ui_tog_') || name.startsWith('ui_check_') || name.startsWith('ui_radio_')
      hash = { type: 'Toggle' }
    else if name.startsWith('ui_drop_')
      hash = { type: 'Dropdown' }
    else if name.startsWith('ui_scroll_')
      hash = { type: 'ScrollRect' }
      hash['scroll'] = opt['scroll'] if opt['scroll']
    else if name.startsWith('ui_tab_')
      hash = { type: 'ToggleGroup' }
    else if name.startsWith('ui_panel_') || name.startsWith('ui_card_') || name.startsWith('ui_popup_') || name.startsWith('ui_toast_') || name.startsWith('ui_loading_') || name.startsWith('ui_item_') || name.startsWith('ui_header_') || name.startsWith('ui_footer_')
      hash = { type: 'Panel' }
    else if name.startsWith('ui_mask_')
      hash = { type: 'Mask' }
    else if name.startsWith('ui_glg_')
      hash = { type: 'GridLayoutGroup' }
    else if name.startsWith('ui_vlg_')
      hash = { type: 'VerticalLayoutGroup' }
    else if name.startsWith('ui_hlg_')
      hash = { type: 'HorizontalLayoutGroup' }
    else if name.startsWith('ui_cg_')
      hash = { type: 'CanvasGroup' }
    else
      hash = { type: 'Group' }
...
class Util
  ...
  @layerToImageNameLoop: (layer) ->
    ...
    # 清理字符
    baseName.replace('_', '').replace(' ', '-').toLowerCase()
