...
Shader "UI/RoundedClip"
{
    Properties
    {
        [PerRendererData] _MainTex ("Sprite Texture", 2D) = "white" {}
        _Color ("Tint", Color) = (1,1,1,1)
        _Radius ("Radius", Range(0,0.5)) = 0.12
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f i):SV_Target
            {
                ...
                float2 p=abs(i.texcoord-0.5);
                float2 hs=float2(0.5-_Radius,0.5-_Radius);
                float2 q=p-hs;
                float d=length(max(q,0.0))+min(max(q.x,q.y),0.0)-_Radius;
                float mask=1.0-smoothstep(0.0,_Softness,d);
                col.a*=mask;
