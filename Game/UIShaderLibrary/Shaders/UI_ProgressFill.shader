...
Shader "UI/ProgressFill"
{
    Properties
    {
        ...
        _FillColor ("Fill Color", Color) = (0.2,1,0.4,1)
        _BackColor ("Back Color", Color) = (0.2,0.12,0.08,1)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 _Color;
            float4 _ClipRect;
            float _FillAmount; fixed4 _FillColor; fixed4 _BackColor; float _Direction;
            ...
            fixed4 frag(v2f i):SV_Target
            {
                ...
                float axis=i.texcoord.x;
                if(_Direction>=0.5 && _Direction<1.5) axis=1.0-i.texcoord.x;
                else if(_Direction>=1.5 && _Direction<2.5) axis=i.texcoord.y;
                else if(_Direction>=2.5) axis=1.0-i.texcoord.y;
                float m=step(axis,_FillAmount);
                fixed4 part=lerp(_BackColor,_FillColor,m);
                col*=part;
